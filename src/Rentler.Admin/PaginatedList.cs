...
namespace Rentler.Admin
{
    ...
    public class PaginatedList<T>:List<T>
    {
        ...
        /// <summary>
        /// Clones all of the items within the paginated list.
        /// </summary>
        /// <returns>A completely new paginated list with the
        /// same results.</returns>
        public PaginatedList<T> DeepClone()
        {
            //var x = this.Select(item => (T)((ICloneable)(item)).Clone()).AsEnumerable();
            var x = this.Select(item => (T)item).AsEnumerable();
